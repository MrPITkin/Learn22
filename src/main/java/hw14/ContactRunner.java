...
//        long seconds = TimeUnit.SECONDS.convert(spentTime, TimeUnit.MILLISECONDS);
        ContactWithOthers[] contacts = generateElementsInArrayAndMeasureTime
                (ContactRunner::fillContactArrays);
        Collection<ContactWithOthers> contactsList =
                generateElementsAndMeasureTime(ContactRunner::fillContactList);
        Collection<ContactWithOthers> contactsSet =
                generateElementsAndMeasureTime(ContactRunner::fillContactSet);

        System.out.println("-*-*-*-*-*-*-");
        fillLinkedContacts(contactsList);
        System.out.println("-*-*-*-*-*-*-");
        fillLinkedContacts(contactsSet); // исполнение на 100 контактах > 3 мин.
/*
        try {
            Thread.sleep(5500); // пауза в мс, чтобы увидеть консоль
...
*/
        ...
        Map<Contact, Collection<Contact>> mapOfContactsWithLinkedContacts =
                contactsList.stream().collect(Collectors.toMap(Contact::new,
                        ContactWithOthers::getLinkedContacts));
//        System.out.println(mapOfContactsWithLinkedContacts); //забивает инфу в консоли

        Map<Contact, Integer> mapOfContacts = contactsList.stream().map
                        (ContactWithOthers::getLinkedContacts).flatMap(Collection::stream).
    ...
    private static void fillLinkedContacts(Collection<ContactWithOthers> contactsList) {
        ...
        for (ContactWithOthers contact : contactsList) {
            IntStream.range(0, LIMIT_OF_LINKED_CONTACTS).boxed().map(i -> randomElement
                    (contactsList)).forEach(contact.getLinkedContacts()::add);
        }
        long spentTime = System.currentTimeMillis() - now;
        System.out.printf("Заполнение связанных контактов %s из %d элементов потребовало" +
                " %.3f секунд\n", contactsList instanceof List ? "списка" : "множества",
                LIMIT_OF_LINKED_CONTACTS, spentTime / 1000d);
    }

    public static ContactWithOthers[] generateElementsInArrayAndMeasureTime
            (Supplier<ContactWithOthers[]> supplier) {
        long now = System.currentTimeMillis();
    ...
    public static Collection<ContactWithOthers> generateElementsAndMeasureTime
            (Supplier<Collection<ContactWithOthers>> supplier) {
        long now = System.currentTimeMillis();
